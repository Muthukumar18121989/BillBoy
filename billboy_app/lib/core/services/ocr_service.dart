...
class OcrService {
  Future<OcrExtractedData> extractFromImage(String imagePath) async {
    // OCR scanning disabled — requires google_mlkit_text_recognition
    return OcrExtractedData(rawText: '', confidence: 0);
  }
  ...
  String? _extractProductName(List<String> lines) {
    for (final line in lines.take(5)) {
      if (line.length > 5 && !RegExp(r'^\d').hasMatch(line)) return line;
    }
    return null;
  }
  ...
  String? _extractGst(String text) {
    return RegExp(r'[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}').firstMatch(text)?.group(0);
  }
  ...
  String? _extractSerial(String text) {
    return RegExp(r'(?:serial|s/n|s\.n\.)[:\s]*([A-Z0-9]{8,})', caseSensitive: false).firstMatch(text)?.group(1);
  }

  String? _extractImei(String text) {
    return RegExp(r'IMEI[:\s]*(\d{15})').firstMatch(text)?.group(1);
  }

  int? _extractWarranty(String lowerText) {
    final yearMatch = RegExp(r'(\d+)\s*year\s*warrant', caseSensitive: false).firstMatch(lowerText);
    if (yearMatch != null) return (int.tryParse(yearMatch.group(1)!) ?? 0) * 12;
    final monthMatch = RegExp(r'(\d+)\s*month\s*warrant', caseSensitive: false).firstMatch(lowerText);
    if (monthMatch != null) return int.tryParse(monthMatch.group(1)!);
    return null;
  }
